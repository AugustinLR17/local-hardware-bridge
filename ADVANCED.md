...
Local Hardware Bridge has a built-in ability to generate a self-signed certificate.
...
Local Hardware Bridge is usually listening on 127.0.0.1. It's normally impossible to obtain valid certificates signed for that.

A common workaround is to point your (sub-)domain A Record to 127.0.0.1, and obtain certificate with that

e.g. Point `local.example.com` to `127.0.0.1`, then point your WebApp to `wss://local.example.com:12212`
